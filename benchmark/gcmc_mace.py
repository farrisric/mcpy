"""GCMC benchmark — MACE calculator.

Run alongside gcmc_alchemi.py (same seeds, same system) to compare wall time.
Total time includes model loading + GCMC run.

Usage:
  conda activate alchemi
  python benchmark/gcmc_mace.py
  python benchmark/gcmc_mace.py --steps 100 --outdir benchmark_results
"""
import time
...
# ── shared config (must match gcmc_alchemi.py) ──────────────────────────────
MASTER_SEED = 42
STEPS = 100
T_K = 500.0
MU_AG = -2.99
...
def parse_args():
    ...
    p.add_argument('--model-path', default=None,
                   help='Local MACE .pt path; omit to use --checkpoint')
    p.add_argument('--checkpoint', default='medium-mpa-0',
                   help='Named MACE-MP checkpoint (matches gcmc_alchemi.py --checkpoint)')
    p.add_argument('--fmax', type=float, default=0.05)
    p.add_argument('--relax-steps', type=int, default=500)
    p.add_argument('--optimizer', choices=['lbfgs', 'fire'], default='lbfgs',
                   help='ASE optimizer for relaxation. Use "fire" for apples-to-apples '
                        'comparison with AlchemiFCalculator (also FIRE).')
...
def main():
    ...
    print(f'MACE benchmark  |  steps={args.steps}  device={args.device}  seed={MASTER_SEED}')

    if args.model_path:
        base = args.model_path
    else:
        base = mace_mp(model=args.checkpoint, device=args.device)
    ...
    gcmc = GrandCanonicalEnsemble(
        ...
        outfile=os.path.join(args.outdir, 'mace.out'),
        traj_file=os.path.join(args.outdir, 'mace.xyz'),
    ...
    print('\n' + '=' * 50)
    print(f'  MACE results — {args.steps} steps')
    print('=' * 50)
    print(f'  Optimizer:       {calculator.optimizer_name.upper()}')
    print(f'  GCMC run time:   {gcmc_s:>8.2f} s  ({gcmc_s/args.steps*1000:.1f} ms/step)')
    print(f'  Total wall time: {total_s:>8.2f} s  (incl. model loading)')
    ...
    print(f'\n  Output: {os.path.abspath(args.outdir)}/mace.out  /  mace.xyz')
