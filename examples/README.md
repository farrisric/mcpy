...
| `gcmc_ag111.py` | A production run: O on Ag(111) with a machine-learned potential, FIRE relaxation, chemical potentials derived from the potential itself. | `nvalchemi-toolkit[mace]`, GPU | hours |
| `re_gcmc_batched.py` | Replica exchange on one GPU, with every replica's trial energy evaluated in a single batched forward pass. | `nvalchemi-toolkit[mace]`, GPU | hours |
